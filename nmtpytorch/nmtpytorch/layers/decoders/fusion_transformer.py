...
from ..transformer import PositionalEncoding, MultiHeadAttention, FusedMultiHeadAttention, PositionwiseFeedForward
...
class DecoderLayer(nn.Module):
    def __init__(self, model_dim, feat_dim, feat_ratio, key_size, n_heads, inner_dim, dropout, dropnet):
        ...
        self.ctx_attn = FusedMultiHeadAttention(model_dim, feat_dim, feat_ratio, n_heads, 
            dropout=dropout, dropnet=dropnet)
        self.ff = PositionwiseFeedForward(model_dim, inner_dim, dropout=dropout)

        self.layer_norm = nn.LayerNorm(model_dim)

    def forward(self, hs, enc_hs, feat, self_attn_mask, ctx_attn_mask, feat_attn_mask=None):
...
class FusionTransformerDecoder(nn.Module):
    ...
    def f_next(self, ctx_dict, y, h, training=False):
        ...
        feat = ctx_dict[self.feat_name][0].transpose(0, 1)
        ...
        for t, layer in zip(self.layer_types, self.layers):
            if t == 't':
                ...
                hs, self_attn, ctx_attn = layer(hs, enc_hs, feat, self_attn_mask, ctx_attn_mask)
