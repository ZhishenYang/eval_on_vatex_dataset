...
class ConditionalMMDecoder(ConditionalDecoder):
    ...
    def __init__(self, fusion_type='concat',
                 use_feat_pe=False, feat_max_len=50, **kwargs):
        ...
        self.img_att = Attention(
            self.ctx_size_dict[self.feat_name], self.hidden_size,
        ...
        if use_feat_pe:
            self.img_pe = PositionalEncoding(
                model_dim=self.ctx_size_dict[self.feat_name],
                max_len=feat_max_len)
            self.img_emb = lambda ctx: [self.img_pe(ctx[0].transpose(0,1)).transpose(0,1), ctx[1]]
        else:
            self.img_emb = lambda ctx: [ctx[0], ctx[1]]
    ...
    def f_next(self, ctx_dict, y, h):
        ...
        self.img_alpha_t, img_z_t = self.img_att(
            h1.unsqueeze(0), *self.img_emb(ctx_dict[self.feat_name]))
