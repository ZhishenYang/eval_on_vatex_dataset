...
class EncoderLayer(nn.Module):
    def __init__(self, model_dim, feat_dim,
                 key_size, n_heads, inner_dim, dropout):
        super(EncoderLayer, self).__init__()

        self.attn = MultiHeadAttention(model_dim, n_heads, dropout=dropout)
        self.fuse = MultiHeadAttention(model_dim, n_heads, dropout=dropout,
                                       k_dim=feat_dim, v_dim=feat_dim)
        self.ff = PositionwiseFeedForward(model_dim, inner_dim, dropout=dropout)

        self.layer_norm = nn.LayerNorm(model_dim)

    def forward(self, x, attn_mask, pad_mask, z):
        h1, attn = self.attn(x, x, x, attn_mask)
        h1 = self.layer_norm(h1 + x)
        h1 = h1.masked_fill(pad_mask, 0)

        h2, _ = self.fuse(h1, z, z)
        h2 = self.layer_norm(h2 + h1)
        h2 = h2.masked_fill(pad_mask, 0)

        h3 = self.ff(h2)
        h3 = self.layer_norm(h3 + h2)
        h3 = h3.masked_fill(pad_mask, 0)

        return h3, attn

class StackedMultimodalTransformerEncoder(nn.Module):
    def __init__(self, n_vocab, model_dim, feat_dim, max_len, 
        n_layers, key_size, n_heads, inner_dim, dropout, **kwargs):
        super(StackedMultimodalTransformerEncoder, self).__init__()
    ...
    def forward(self, x, z, **kwargs):
        # x, z: len x batch_size (default order in nmtpytorch)
        ...
        attns = []
        for layer in self.layers:
            hs, attn = layer(hs, attn_mask, pad_mask, z)
            attns.append(attn)
