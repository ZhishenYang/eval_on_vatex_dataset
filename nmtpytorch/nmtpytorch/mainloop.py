...
class MainLoop:
    def __init__(self, model, train_opts, dev_mgr):
        ...
        if train_opts.get('enc_pretrained_emb', None):
            emb = model.get_enc_emb()
            emb.weight.data = torch.Tensor(np.load(train_opts['enc_pretrained_emb']))
        if train_opts.get('dec_pretrained_emb', None):
            emb = model.get_dec_emb()
            emb.weight.data = torch.Tensor(np.load(train_opts['dec_pretrained_emb']))
